...
function formatData(data) {
    ...
    for (var i = 0; i < sortedData.length; i++) {
        ...
        if (sortedData[i].Time) {
            var formattedTime = fix(sortedData[i].Time.getHours()) + ":" + fix(sortedData[i].Time.getMinutes()) + ":" + fix(sortedData[i].Time.getSeconds());
...
function formatDate(data) {
    for (let i = 0; i < data.length; i++) {
        data[i].Time = new Date(2019, parseInt(data[i].Time.substring(8, 10) -1), parseInt(data[i].Time.substring(6, 8)), parseInt(data[i].Time.substring(0, 2)), parseInt(data[i].Time.substring(2,4)), parseInt(data[i].Time.substring(4,6)))
    }
    return data;
}
